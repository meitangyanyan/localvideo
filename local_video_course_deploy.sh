...
sed=/bin/sed
CA_DIR=/etc/pki/CA
echo -n  "please input your all course_id(the separator is '|'): "
read course_id
echo -n  "please input your ip: "
read ip
[ ! -f RPM-GPG-KEY-GXB -a -f rpm-gpg-key-gxb ] && mv rpm-gpg-key-gxb  RPM-GPG-KEY-GXB
pids=`ps aux|grep -E '(httpd|nginx|deploy_course|deploy_school)'|grep -v grep|awk '{print $2}'` 
[ -z "$pids" ] || /bin/kill -9 $pids
...
sed -i "s/course_id = .*/course_id = ${course_id}/" /home/kkb/videolocaldeploy/conf/setting.conf
...
echo '0 1 * * * /usr/bin/python /home/kkb/videolocaldeploy/deploy_course.py &> /home/kkb/videolocaldeploy/log/$(date "+\%Y-\%m-\%d").log &' >> /var/spool/cron/kkb
echo '0 0 * * * /bin/find /home/kkb/videolocaldeploy/log -name "*-*-*.log" -mtime +7 -exec rm -f {} \; &>/dev/null &' >> /var/spool/cron/kkb
/bin/su - kkb -c 'python /home/kkb/videolocaldeploy/deploy_course.py >> /home/kkb/videolocaldeploy/log/$(date "+%Y-%m-%d").log 2>&1' &
...
/bin/ps aux| grep deploy_course | grep -v grep &> /dev/null
if [ $? -eq 0 ]
then
